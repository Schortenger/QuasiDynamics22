classdef ROSHelper < handle
	...
	methods
		...
		function out = setHandsEqualHeight(obj, hand_ref)

			if nargin < 2; hand_ref = 1; end;

			obj.clearBuffers();
			getret1 = obj.getCartesian(1);
			getret2 = obj.getCartesian(2);

			diffz = getret1.Z - getret2.Z;

			if hand_ref == 1
				getret = obj.getCartesian(2);
				out = obj.setCartesian(2, getret.X, getret.Y, getret.Z + diffz, getret.Q0, getret.Qx, getret.Qy, getret.Qz);
			else
				getret = obj.getCartesian(1);
				out = obj.setCartesian(1, getret.X, getret.Y, getret.Z - diffz, getret.Q0, getret.Qx, getret.Qy, getret.Qz);
			end
		end
